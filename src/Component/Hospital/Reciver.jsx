...
const Donor = () => {
  ...
  const fetchDonor = async()=>{
    try {
      const {data} = await axios.get(`${server}/hospital/getreceiver`,{
        withCredentials:true
      })
      setDonor(data.receiver)
    } catch (error) {
      console.log(error);
    }
   }
   ...
   const toggleStatus = async (id,status)=>{
    ...
    try {
     ...
     const {data}= await  axios.get(`${server}/hospital/togglestatus/receiving/${id}`,{
        withCredentials:true
      })
      // console.log(data);
      setFetch(!fetch)
...
fetchDonor()
          ...
          const rows = [
            { id: 1, uid:1,string: 'John Doe', Age: 30, "Contact Number": '123-456-7890', 'bloodgroup':'A+', Quantity: 20, 'Donation Year': 2022,'Status':"Received" },
            { id: 2, uid:1,string: 'Jane Smith', Age: 25, "Contact Number": '987-654-3210', 'bloodgroup': 'B+', Quantity: 15, 'Donation Year': 2021,'Status':"Received" },
            { id: 3, uid:1,string: 'Bob Johnson', Age: 35, "Contact Number": '555-555-5555', 'bloodgroup': 'O-', Quantity: 10, 'Donation Year': 2022 ,'Status':"Received" },
            { id: 4, uid:1,string: 'Alice Brown', Age: 28, "Contact Number": '111-222-3333', 'bloodgroup': 'AB+', Quantity: 30, 'Donation Year': 2023 ,'Status':"Received" },
          ];
        let cnt=10;
        for(const i in donor){
          console.log(donor[i].receiver._id)
             rows.push({
           id:cnt,
           uid:donor[i].receiver._id,
           string:donor[i].receiver.name,
           Age:donor[i].receiver.age,
           "Contact Number":donor[i].receiver.phone,
           bloodgroup:donor[i].receiver.bloodgrp,
        ...
        renderCell: (params) => {
            ...
            return (
                <strong className='toggleStatus' onClick={()=>toggleStatus(params.row.uid,params.row.Status)}>
                    {status == 'Received' ? <FcApprove/> : <MdOutlinePendingActions />}
                    {/* {params.row.Status} */}
                </strong>
            );
        },
